...
V14-B 原有步骤（保持不变）
────────────────────────────────────────
S1 OpenAlex Enrich → S2 SPC Main Path → S3 KeystoneScore V14
→ S4 子图构建 ~3K → S5a SciBERT 引用分类

         ┌──── 论文级预测（V14-B 已有）────┐   ┌── 概念级预测（★ 新增）──┐
         ...
         └──────────────────────────────┘   └────────────────────────┘
                        ...
                        ▼              ▼               ▼              ▼
              ┌─────────────────────────────────────────────────────────┐
              │     S6 四路 Meta-learning Stacking 融合（★ 升级）        │
...
| 原则 | 实现方式 |
|------|---------|
| 数据复用 | S5d/S5e 直接读 V14-B 的 SQLite（papers 表 title+abstract），无需重新爬取 |
| 嵌入复用 | V14-B S5b 已算 all-mpnet-base-v2 embedding，S5d 概念提取可直接复用 |
| 存储隔离 | 新增 4 张表写入 v14_pilot.sqlite3，与 V14-B 其他表并列，不动原有代码 |
| 代码隔离 | 新代码位于 `echelon/semnet/` 模块，与 `echelon/v14b/` 平行，不修改 V14-B 原有模块 |
| 融合接口 | step6_fusion.py 是唯一修改点：新增加载 predicted_concept_links 表，接入第四路输入 |
...
- **嵌入模型**：all-mpnet-base-v2（复用 V14-B 已加载的模型）
...
## 5. S6 四路 Meta-learning Stacking 融合
...
### 5.2 Meta-learning Stacking 设计

**替代原有的简单交集/加权融合**，采用 Random Forest 作为元学习器：
...
### 6.1 新增表（4 张，写入 v14_pilot.sqlite3）
...
├── semnet/                      # ★ 新增模块
│   ├── __init__.py
│   ├── config.py                # SEMNET 配置（阈值、模型超参等）
│   ├── step5d_concept_extract.py  # 概念提取流水线
│   ├── step5e_concept_network.py  # 共现网络构建 + 特征工程
│   ├── step5e_link_predict.py     # GBDT + NN 链接预测
│   ├── concept_normalize.py       # 同义词归一化
│   └── feature_engineering.py     # 概念对特征计算
├── v14b/                        # V14-B 现有模块（不修改）
│   ├── step6_fusion.py          # 唯一修改点：三路→四路融合
│   └── ...
└── ...
```
...
| 集成点 | 文件 | 修改内容 |
|--------|------|---------|
| 融合升级 | `echelon/v14b/step6_fusion.py` | 新增加载 predicted_concept_links 表，接入第四路，将简单交集替换为 Meta-learning Stacking |
| 流水线编排 | `run_pilot_v5.py` 或新增 `run_v15.py` | 在 S5a 之后并行调度 S5d/S5e（与 S5b/S5c 并行） |
| 配置 | `echelon/v14b/config.py` | 新增 SEMNET 相关配置项 |

---

## 9. 文献支撑矩阵（仅含已验证的 A*/A 级论文）
...
| 依赖 | 用途 |
|------|------|
| sentence-transformers (all-mpnet-base-v2) | 概念嵌入 + KeyBERT 后端 |
| torch + torch-geometric | VGAE（V14-B 已有） + KGE 嵌入 |
| networkx | 概念共现图构建 + 拓扑特征计算 |
| scikit-learn | 层次聚类 + RF 元学习器 |
| sqlite3 | 数据存储 |
...
**验证标准**：concept_nodes 表 ≥ 100 个概念，paper_concepts 表覆盖率 ≥ 90%

### Phase 2：链接预测 + 融合升级（第 2 周）
...
**验证标准**：AUC ≥ 0.85，四路融合输出 future_directions 表

### Phase 3（后续）：规模化 + VGAE 升级

- [ ] 扩展到 20 万篇（光学 + AI）
- [ ] V14-B VGAE → D-VGAE 升级
- [ ] 性能优化（批处理、增量更新）
- [ ] K8s 部署到火山引擎
...
**决策**：Phase 1 保持现有 VGAE，Phase 3 升级到 D-VGAE  
**依据**：D-VGAE (WWW 2024, A*) 解耦 homophily/popularity，解决标准 VGAE 内积解码器根本缺陷  
**不急于升级的理由**：SEMNET 是独立的第四路，不依赖 VGAE 性能；先完成四路融合框架，再优化单路性能
